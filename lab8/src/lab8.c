...
static void add_word_counts_in_chunk(count_map_t *map, word_t *words,
  ...
  for (size_t i = 0; i < num_words; i++) {
    ...
    // TODO: lock the mutex before doing anything
    ...
    // unlock the mutex after modifying is done
...
static count_map_t count_words_parallel(word_t *words, size_t num_words) {
  ...
  pthread_mutex_init(&count_mutex, NULL); // initialize count_mutex
  ...
  for (size_t i = 0; i < THREAD_COUNT; i++) {
    ...
    // first set each thread arguments by packing them with function
    threads_args[i] =
        pack_args(&map, thread_arg_words, thread_arg_num_words, &count_mutex);
    // then call create thread function
    int thr = pthread_create(&threads[i], NULL, counter_thread_func,
                             (void *)threads_args[i]);

    if (thr != 0) { // print message if failed to create thread
      fprintf(stderr, "Thread failed to create %d\n", thr);
      exit(EXIT_FAILURE);
    }
  }

  // TODO: Wait for threads to finish
  for (size_t i = 0; i < THREAD_COUNT;
       i++) { // join thread and free thread argument
    pthread_join(threads[i], NULL);
    free(threads_args[i]);
  }

  // TODO: Cleanup
  pthread_mutex_destroy(&count_mutex); // call mutex destroy function
...
int main(void) {
  ...
  if (word_map) {
    ...
    // TODO
    HASH_SORT(word_map, sort_func); // pass map and sort function to HASH_SORT
