...
void map(Input *input, IntermediateInput *intermediate_input) {
  ...
  // copy and double respective values
...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  // check if output for number already made, if so update vals and return
  for (int i = 0; i < *result_count; i++) {
    assert(i < MAX_INPUT - 1);
  ...
  // if not found in output array, set new output in array for the value
  assert(*result_count < MAX_INPUT - 1);
...
void reduce(Output *output) {
  ...
  //
  // print the output object in specified format
