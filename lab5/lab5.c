...
//  next node;
//
//  this assumes some way to calculate where the freed blocks endptr is, whether
//  by size of block given or endptr given or something. sides is there since if
//  both sides of the block have been coalesced, there cant be more done, and we
//  can break out of it
//
//  on sample given: n->startptr == z->endptr, so combine n and z, then b isnt
//  connected at all, then m->endptr == z->startptr so combine m and z, so end
//  up with b block and one new big free block
